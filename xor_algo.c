...
unsigned int xor_algo(void *buffer, long long int len, unsigned int xor_key_interval_start, unsigned int xor_key_interval_len) {
    ...
    unsigned char *xor_key_interval = (unsigned char *)(xor_key_bytes + xor_key_interval_start);
    unsigned char xor_key_byte;
    for (long long int i = 0; i < len; i++) {
	xor_key_byte = *(unsigned char *)(xor_key_interval + (i % xor_key_interval_len));
	*(unsigned char *)(buffer+i) = *(unsigned char *)(buffer+i) ^ xor_key_byte;
    }
    return XOR_ALGO_OK;
}
